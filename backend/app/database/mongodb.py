...
def get_database() -> AsyncIOMotorDatabase:
    ...
    if mongodb.db is None:
        raise RuntimeError(
            ...
            "Appelez connect_to_mongo() au démarrage."
        )
...
async def _ensure_indexes(
    ...
    await db.event_statuses.create_index(
        [
            ("event_id", 1),
            ("user_id", 1),
        ],
    ...
    await db.event_reactions.create_index(
        [
            ("event_id", 1),
            ("user_id", 1),
        ],
    ...
    await db.comment_reactions.create_index(
        [
            ("comment_id", 1),
            ("user_id", 1),
        ],
